...
export class AssetProcessor {
  ...
  manifest
  // Mobile-first sizing strategy with quality preservation
  RULES = {
    '.jpg': { outputs: ['webp', 'avif'], sizes: [400, 800, 1200] },  // Mobile, tablet, desktop
  ...
  async optimizeImage(inputPath, size, format) {
    try {
      ...
      if (size !== 'original') {
        // For square images or portraits, resize both dimensions proportionally
        pipeline = pipeline.resize(size, size, {
          fit: 'inside',  // Preserves aspect ratio, image fits within size
          withoutEnlargement: true,
          kernel: sharp.kernel.lanczos3  // Better quality downsampling
        })
        // Add slight sharpening after resize to maintain crispness
        .sharpen({ sigma: 0.5 })
      }
      
      // Strip unnecessary metadata to reduce file size
      // Keep orientation to avoid rotation issues
      pipeline = pipeline.rotate() // Auto-rotate based on EXIF
        .withMetadata({  // Remove metadata except orientation
          orientation: undefined  // This removes EXIF but keeps orientation applied
        })

      // Convert to format with size-aware quality settings
      // Smaller images can have higher quality (smaller file size anyway)
      // Larger images need more compression
      if (format === 'webp') {
        const quality = size <= 400 ? 90 :   // Mobile: highest quality (small file anyway)
                       size <= 800 ? 85 :   // Tablet: very good quality
                       80;                  // Desktop: good quality with compression
        pipeline = pipeline.webp({ 
          quality,
          effort: 6,
          smartSubsample: true,
          nearLossless: size <= 400  // Better quality for mobile
        })
      } else if (format === 'avif') {
        const quality = size <= 400 ? 75 :   // Mobile: great quality in AVIF
                       size <= 800 ? 65 :   // Tablet: very good quality
                       55;                  // Desktop: still looks great with AVIF
        pipeline = pipeline.avif({ 
          quality,
          effort: 6,
          chromaSubsampling: size > 800 ? '4:2:0' : '4:4:4'  // Better color for smaller sizes
        })
      }
